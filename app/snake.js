class Snake {
  ...
  startWatch() {
    var self = this;
    setInterval(function moveSnake() {
      self.move();
    }, 100);
  }
  ...
  move() {
    ...
    case 1:
      ...
      break;
    case 2: this.body.unshift(this.body[0] + Math.sqrt(this.tiles.length));
      break;
    default:
      this.body.unshift(this.body[0] - 1);
    }
  ...
  generateSnake() {
    this.pos = this.tiles.length / 2 + Math.round(Math.sqrt(this.tiles.length) / 2);
  ...
  watchKeys() {
    var self = this;
    document.body.addEventListener('keydown', function switchDirection(ev) {
      switch (ev.keyCode) {
      case 38:
        if (self.direction !== 2) {
          self.direction = 0;
